...
function extendAndFreezeObject(obj1, obj2) {
  ...
  const extend = Object.assign(obj1, obj2);
  const freezeObject = Object.freeze(obj1);
  if (freezeObject === extend) {
    return freezeObject;
  } else {
    return "Error";
  }
...
function filterObjectsByKey(arr, key) {
  ...
  const filterObject = arr.filter((arr) => {
...
function sealAndCheck(obj) {
  ...
  const sealObject = Object.seal(obj);
  const checkObject = Object.isSealed(sealObject);
  return checkObject;
...
function checkOwnership(obj1, obj2, key) {
  const check1 = obj1.hasOwnProperty(key);
  const check2 = obj2.hasOwnProperty(key);
  if (check1 === check2) {
    return true;
